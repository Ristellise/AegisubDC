...
of {CPP} runtime library. B2 supports using that library on
...
build configuration when B2 selects a Python configuration to use.
...
`command` is the quickbook executable. If it is not specified, B2
...
B2 will search for it in PATH and FOP_HOME.
...
Sometimes the paths produced by B2 can become excessively long.
